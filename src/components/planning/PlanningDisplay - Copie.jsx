...
const PlanningDisplay = ({ config, selectedShop, selectedWeek, selectedEmployees, planning: initialPlanning, setCurrentPlanning, onBack, onBackToShop, onBackToWeek, onBackToConfig, onReset, handleExportToJson, handleResetData }) => {
    ...
    useEffect(() => {
        console.log('Initial planning state:', { initialPlanning, selectedEmployees, selectedWeek, config });
        setPlanning(prev => {
            ...
            if (!config?.timeSlots || !Array.isArray(config.timeSlots) || config.timeSlots.length === 0) {
                console.log('Invalid config, skipping planning initialization');
            ...
            (storedSelectedEmployees || []).forEach(employee => {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    if (!updatedPlanning[employee][dayKey]) {
                        ...
                        const newSlotCount = config.timeSlots.length;
            ...
            if (Object.keys(updatedPlanning).length > 0 && config?.timeSlots?.length > 0) {
                ...
                setCurrentPlanning(updatedPlanning); // Mettre à jour le planning dans App.jsx
            }
            return updatedPlanning;
        });
    }, [selectedEmployees, selectedWeek, config, selectedShop, setCurrentPlanning]);
    ...
    useEffect(() => {
        if (initialPlanning && Object.keys(initialPlanning).length > 0) {
            setPlanning(prev => {
                ...
                const storedSelectedEmployees = loadFromLocalStorage(`selected_employees_${selectedShop}_${selectedWeek}`, selectedEmployees || []);
                if (!config?.timeSlots || !Array.isArray(config.timeSlots) || config.timeSlots.length === 0) {
                    console.log('Invalid config, skipping initial planning synchronization');
                    return updatedPlanning;
                }
                (storedSelectedEmployees || []).forEach(employee => {
    ...
    }, [initialPlanning, selectedWeek, config, selectedShop, setCurrentPlanning]);
    ...
    useEffect(() => {
        if (Object.keys(planning).length > 0 && config?.timeSlots?.length > 0) {
            ...
            setCurrentPlanning(planning); // Mettre à jour le planning dans App.jsx
            console.log('Saved planning to localStorage:', { key: `planning_${selectedShop}_${selectedWeek}`, planning });
        }
    }, [planning, selectedShop, selectedWeek, setCurrentPlanning]);
    ...
    const confirmReset = () => {
        ...
        if (!config?.timeSlots || !config.timeSlots.length) {
            setFeedback('Erreur: Configuration des tranches horaires non valide.');
            return;
        }
        ...
        return (
            <div className="planning-container">
                ...
                <div className="navigation-buttons">
                    ...
                    <Button className="button-base button-primary" onClick={handleExportToJson}>
                        Exporter JSON
                    </Button>
                    <Button className="button-base button-reinitialiser" onClick={handleResetData}>
